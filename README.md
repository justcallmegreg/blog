...
2. **Give the overseer these secrets** (via `overseer.existingSecret` / `overseer.env`) — the public
   ...
   | `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` | R2 API-token credentials with **`PutObject` (upload) and `DeleteObject` scope** — the overseer uploads video presigned URLs and deletes media when transmission entries are removed. |
   ...
   **Never put the `R2_*` credentials on the public `blog-engine` deployment**, and keep its
   ...
   serves R2 objects for HLS playback via `transmissions.mediaBaseUrl`.
